...
class seirdp():
  def __init__(self, r0, r1, gamma, sigma, baseAlpha, fatalityRateAvg, birthRateDaily=0.0, natDeathRateDaily=0.0, socDistResponseFactor=1.0, diseaseScalingFactor=0.0):
    # Assign variables
    self.r0 = r0
    self.r1 = r1
    self.gamma = gamma
  ...
  def model(self, Y, x, N, r0, thrPop, thrDay, r1, gamma, sigma, pho):
    ...
    dI = sigma * E - (1 - alpha) * gamma * I - alpha * pho * I + self.birthRateDaily * I
    dR = (1 - alpha)*gamma * I - self.natDeathRateDaily * R + self.birthRateDaily * N
    dD = alpha*pho*I
...
if __name__ == "__main__":
  ...
  modelInstance = seirdp(covid_params.r0, covid_params.r1, covid_params.GAMMA, covid_params.SIGMA, covid_params.BASE_ALPHA_RATE, covid_params.FATALITY_RATE_AVERAGE,
  ...
  X, S, E, I, R, D, P = modelInstance.solve(covid_params.POPULATION, E0, covid_params.SOCIAL_DISTANCE_THRESHOLD_POPULATION, covid_params.SOCIAL_DISTANCE_DAY, covid_params.DAYS_MODEL)
  ...
  print('Number of Dead due to Disease at Day ' + str(covid_params.DAYS_MODEL) + ': ' + str(D[-1]))
  print('Number of Dead due to "natural causes" at Day ' + str(covid_params.DAYS_MODEL) + ': ' +str( P[-1]))
